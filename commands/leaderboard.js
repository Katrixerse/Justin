...
exports.run = async (client, message, args) => {
    ...
    if (choice === "cash" || choice === "money") {
        sql.all(`SELECT username,userId,cash,bank FROM user_profiles WHERE guildId ="${message.guild.id}" ORDER BY cash DESC LIMIT 10`).then(rows => {
        ...
        sql.all(`SELECT username,userId,level,xp FROM user_profiles WHERE guildId ="${message.guild.id}" ORDER BY xp DESC LIMIT 10`).then(rows => {
