...
exports.run = async (client, message, args) => {
  if (!message.guild.member(client.user).hasPermission('ATTACH_FILES')) return message.reply('Sorry, i dont have the perms to do this cmd i need ATTACH_FILES. :x:')
   const { Canvas } = require('canvas-constructor');
    if (message.mentions.users.size < 1) return message.channel.send("You didn't mention a user to put a rainbow over them.");
   const getSlapped = async (person) => {
    ...
    await message.channel.send({ files: [{ attachment: result, name: 'pride.png' }] });
  } catch (error) {
    throw error;
  }
}
...
exports.help = {
    name: 'pride',
    description: 'Puts a raindow over a users avatar [CANVAS].',
    usage: 'pride @User'
};